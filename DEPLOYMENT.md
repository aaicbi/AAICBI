...
| "Sign in with Google" redirects to Google, then comes back to an error | `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` missing, or the exact `{APP_URL}/api/auth/google/callback` URL isn't registered as an Authorized redirect URI for this OAuth client in Google Cloud Console | Double-check both — Google rejects the callback outright if the redirect URI doesn't match byte-for-byte (including http vs. https and trailing slashes) |
...
| `CRON_SECRET` | Generate one yourself: `openssl rand -base64 32`. Secures `/api/cron/course-access-sweep` (see `vercel.json`) — Vercel automatically sends this same value as `Authorization: Bearer ...` on its own scheduled invocations once this env var is set, no further configuration needed. Without it, the daily access-expiry sweep and reminder emails simply won't run |
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | From Google Cloud Console → APIs & Services → Credentials → Create Credentials → OAuth client ID (type: Web application). **Add your production `APP_URL` + `/api/auth/google/callback` as an Authorized redirect URI** in that same screen — Google rejects the request otherwise, and this is a genuinely separate value from whatever you registered for local development. Trainee-only "Sign in with Google" on `/trainee/login` and `/trainee/register`; leave both blank if you don't want this feature live yet — the buttons simply fail with a clear error instead of anything crashing. |
