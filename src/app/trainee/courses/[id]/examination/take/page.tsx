...
/**
 * The course-scoped twin of the module assessment take page —
 * deliberately a near-byte-for-byte copy, same reasoning as that
 * file's own comment: the security-critical parts live in the
 * attempt-scoped routes and examEngine.ts, not here. sessionStorage
 * keys off courseId instead of moduleId, since that's what this route
 * is reached with.
 */
export default function TakeCourseExaminationPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const raw = sessionStorage.getItem(`lms_course_exam_${params.id}`);
    const startedRaw = sessionStorage.getItem(`lms_course_exam_start_${params.id}`);
    if (!raw || !startedRaw) {
      router.replace(`/trainee/courses/${params.id}/examination`);
      return;
    }
  ...
  // Optional monitoring (§21) — only wired up when the examination has
  ...
  async function doSubmit() {
    ...
    sessionStorage.setItem(`lms_course_exam_result_${params.id}`, JSON.stringify(data));
    sessionStorage.removeItem(`lms_course_exam_${params.id}`);
    router.push(`/trainee/courses/${params.id}/examination/result`);
  }
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-brand-sand">
      {/* Top bar — timer stays visible on mobile per §22. Logo is
          ...
          reasoning as the module assessment take page: a "way out"
          link during a live, timed, possibly-monitored examination
          would be actively unwelcome. */}
      ...
      {/* Submit confirmation (§39) — same deliberate custom modal as
