...
import { LineChart as GaugeIcon, TrendingUp, TrendingDown, Minus, Download, AlertTriangle, X } from "lucide-react";
...
export default function PerformanceDashboardPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    setRows(null);
    setKpis(null);
    fetch(query(`/api/courses/${params.id}/performance`))
      .then(async (r) => {
        if (!r.ok) {
          setNotFound(true);
          return;
    ...
    fetch(query(`/api/courses/${params.id}/performance/modules`))
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => data && setModuleStats(data.modules))
      .catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [params.id, cohortFilter, moduleFilter]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/courses/${params.id}/performance/trainees/${selectedTraineeId}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => setDetail(data))
      .finally(() => setDetailLoading(false));
  }, [selectedTraineeId, params.id]);
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
          ...
          {!loading && (
            <div className="flex gap-2">
              <Button href={query(`/api/courses/${params.id}/performance/export.csv`)} variant="secondary" size="sm" iconLeft={<Icon icon={Download} size="sm" />}>
                Export CSV
              </Button>
              <Button href={query(`/api/courses/${params.id}/performance/export.pdf`)} variant="secondary" size="sm" iconLeft={<Icon icon={Download} size="sm" />}>
                Export PDF
              </Button>
            </div>
          )}
          ...
          <>
            ...
            <section className="mt-6">
              {filteredRows.length === 0 ? (
                <EmptyState
                  ...
                  title="No trainees match"
                  description="Adjust the filters or search above."
                />
              ) : (
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              ...
                              <span className="text-xs text-gray-500">({r.completionPct}%)</span>
                            </td>
                    ...
                    <tbody>
                      {moduleStats.map((m) => (
                        <tr key={m.moduleId} className="border-b border-gray-100 last:border-0">
                          ...
                          <td className="px-4 py-3">
                            {m.hasAssessment ? <Badge variant="neutral">Configured</Badge> : <Badge variant="warning">Not configured</Badge>}
                          </td>
            ...
            <section className="mt-10">
              ...
              <p className="mt-1 text-xs text-gray-500">
                ...
                <a href={`/admin/courses/${params.id}/early-warnings`} className="font-semibold text-brand-teal hover:underline">
                  Early Warnings
                </a>{" "}
                page is opened.
              </p>
          ...
          <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-brand-surface p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            {detailLoading || !detail ? (
              ...
              <>
                ...
                {detail.isAtRisk && (
                  <div className="mt-4 rounded-lg bg-brand-roseLight px-3 py-2 text-xs text-brand-rose">
                    {detail.atRiskReasons.join(" · ")}
                  </div>
                )}
              </>
            )}
