...
export default function InvestorResetPasswordPage() {
  return (
    <Suspense fallback={null}>
      <InvestorResetPasswordForm />
    </Suspense>
  );
}

function InvestorResetPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError(null);
    const res = await fetch("/api/auth/investor-reset-password", {
      method: "POST",
      ...
      <main className="mx-auto flex min-h-[calc(100vh-73px)] max-w-sm flex-col justify-center px-6">
        ...
        {done ? (
          <Card className="mt-6 text-center">
            <p className="text-sm text-gray-600">Your password has been set.</p>
            <Button onClick={() => router.push("/investor/login")} className="mt-4">
              Sign In
            </Button>
          </Card>
        ) : (
